...
bool trovaIlPianetaDiPartenza_rec(int& identificativoPianetaDiPartenza, int carburanteCorrente, int numeroDiPianeti, int tentativi) {
    ...
    //              (per esempio, "tentativi" e` uguale a "numeroDiPianeti")
        ...
        identificativoPianetaDiPartenza = -1;// 1. settare "identificativoPianetaDiPartenza = -1;"
        return false;// 2. ritornare il valore booleano corretto
    }
    ...
    //              (per esempio, la variabile "identificativoPianetaDiPartenza" ha un valore apposito)
        ...
        identificativoPianetaDiPartenza = identificativoPianetaCorrente;// 1. settare "identificativoPianetaDiPartenza = identificativoPianetaCorrente;"
        ...
        dequeue();
        enqueue(identificativoPianetaCorrente, carburantePianetaCorrente, distanzaPianetaCorrente);// 3. modificare opportunamente la coda (per esempio, spostando il pianeta
        //    corrente in fondo alla coda con le funzioni dequeue/enqueue)
        bool a = trovaIlPianetaDiPartenza_rec(identificativoPianetaDiPartenza, carburanteCorrente, numeroDiPianeti, tentativi);// 4. invocare ricorsivamente la funzione "trovaIlPianetaDiPartenza_rec",
        //    ricordandone il valore di ritorno
        if ((carburanteCorrente>=0) && a) {
            return true;
        }
        else {
    ...
    // INDUZIONE 2: l'astronave sta viaggiando da un pianeta all'altro
        ...
        dequeue();
        enqueue(identificativoPianetaCorrente, carburantePianetaCorrente, distanzaPianetaCorrente);
        bool a = trovaIlPianetaDiPartenza_rec(identificativoPianetaDiPartenza, carburanteCorrente, numeroDiPianeti, tentativi);
        return (a && (carburanteCorrente>=0));
