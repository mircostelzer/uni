...
int conta(int n, list *l2) {
    int res;
    if (l2 == NULL) {
        res = 0;
    }
    else if (n == l2->code) {
        res = 1 + conta(n, l2->next);
    }
    else {
        res = conta(n, l2->next);
    }
...
void estrai_aux(list *l1, list *l2, list *&res) {
    if (l1 == NULL) {
        return;
    }
    list *l = new list;
    ...
    if (l1->code%2!=0) {
        ...
        l->code = conta(l1->code, l2);
    }
    res = l;
    estrai_aux(l1->next, l2, res->next);
...
list *estrai(list *l1, list *l2) {
    ...
    estrai_aux(l1,l2, res);

    return res;
}

list *estrai1(list *l1, list *l2) {
    if (l1 == nullptr)
      return nullptr;
    if (l1->code%2==0)
      return new list { conta(l1->code, l2), estrai(l1->next, l2)};
