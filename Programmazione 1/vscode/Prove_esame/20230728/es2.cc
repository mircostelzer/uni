...
// implementare
list *function(float, int, double);
...
double Taylor(float x, int n) {
    double res = (pow(x, (2*n + 1)))/factorial(2*n + 1);

    return res; 
}


list *function_aux(float x, int N, double e, int i, double prec, list *&res) {
    double curr = prec + Taylor(x, i);
    if (i >= N || (abs(curr - prec) < abs(e * abs(curr)))) {
        return res;
    }
    res = new list {curr, res};
    res = function_aux(x, N, e, i+1, curr, res);
...
list *function(float x, int N, double e) {
    ...
    res = function_aux(x, N, e, 0, 0.0, res);
