...
list *function_aux(float x, int N, int precision, list *res, double prev, int i) {
    double f = prev + pow(x, 2*i+1)/factorial(2*i+1);
    if (i<=N && (abs_val(f - prev)>=abs_val(precision*abs(prev)))) {
        res = new list {f, res};
        return function_aux(x, N, precision, res, f, i+1);
    }
    else {
        return res;
    } 
...
list *function(float x, int N, double precision){
    list *res = NULL;
    res = function_aux(x, N, precision, res, 0.0, 0);
    return res;
}
