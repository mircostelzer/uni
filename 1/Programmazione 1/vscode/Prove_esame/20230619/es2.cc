...
void calcola_lista_medie(const tree *root, list *&res);
...
void calcola_aux(const tree *root, list *&res, int count, double somma) {
    if (root!=NULL) {
        if (root->info%2 == 0) {
            count++;
            somma += root->info;
        ...
        if (root->left == NULL && root->right == NULL) {
            ...
            calcola_aux(root->right, res, count, somma);
        }
        else if (root->right == NULL) {
            if (count!=0) {
                res = new list {somma/count, res};
            }
            calcola_aux(root->left, res, count, somma);
        }
        else {
            calcola_aux(root->left, res, count, somma);
            calcola_aux(root->right, res, count, somma);
